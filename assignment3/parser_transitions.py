...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            self.dependencies.append((self.stack[stack_len-1],self.stack[stack_len-2]))
            self.stack.pop(stack_len-2)
        elif transition == 'RA':
            stack_len = len(self.stack)
            self.dependencies.append((self.stack[stack_len-2],self.stack[stack_len-1]))
            self.stack.pop(stack_len-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        unfinished_parses = [p for p in partial_parses if not(len(p.stack) == 1 and len(p.buffer) == 0)]
