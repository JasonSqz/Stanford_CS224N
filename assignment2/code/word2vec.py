...
def negSamplingLossAndGradient(
    ...
    negSamVectors = outsideVectors[negSampleWordIndices]

    sigOutCenter = sigmoid(u_o @ centerWordVec)
    sigSampleCenter = sigmoid(-negSamVectors @ centerWordVec)

    loss = -np.log(sigOutCenter) - np.sum(np.log(sigSampleCenter))
    gradCenterVec = (sigOutCenter - 1) * u_o + (1 - sigSampleCenter).T @ negSamVectors

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx,:] = (sigOutCenter-1) * centerWordVec

    #We only update outside words that have been sampled
    #If one word is sampled multiple times, double/thrice etc. its gradient
    
    unique, indexs, counts = np.unique(negSampleWordIndices, return_index = True, return_counts = True)
    sigmoidUniqueSamplesCenter = sigSampleCenter[indexs]
    gradOutsideVecs[unique, :] = np.outer(- ((sigmoidUniqueSamplesCenter - 1) * counts), centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        tmp_loss, tmp_center_grad, tmp_outside_grad = word2vecLossAndGradient(centerWordVec,outsideWordIdx,outsideVectors,dataset)

        loss += tmp_loss
        #Center word upgrade only goes to the center word
        gradCenterVecs[word2Ind[currentCenterWord]] += tmp_center_grad
        gradOutsideVectors += tmp_outside_grad
