...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        truncate_len = random.randint(4,int(self.block_size * 7/8)) #Needs to be an integer since we are truncating the sentence later
        if document_len < truncate_len:
            t_doc = document
        else:
            t_doc = document[:truncate_len] #truncate from the begining
        ...
        #Code copied from https://github.com/parachutel/cs224n-stanford-winter2021/blob/working-qanetxl/a5/src/dataset.py
        mean_masked_content_len = len(t_doc) / 4
        noise = random.uniform(-1/8, 1/8) * len(t_doc)
        masked_content_len = mean_masked_content_len + noise

        #Make sure the masked_content_length stay in the interval [1, truncate_len-2]
        ...
        prefix = t_doc[:start_idx]
        masked_content = t_doc[start_idx : start_idx + masked_content_len]
        suffix = t_doc[start_idx + masked_content_len:]
